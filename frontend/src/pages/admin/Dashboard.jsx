...
export default function AdminDashboard() {
  ...
  const [reportView, setReportView] = useState('user');
  ...
  function toggleSelectAll() {
    if (selectedOrderIds.length === ordersData.orders.length) setSelectedOrderIds([]);
    else setSelectedOrderIds(ordersData.orders.map((o) => o.id));
  }
      ...
      <div className="page-wide">
          ...
          <form onSubmit={handleMenuSubmit}>
            <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
              ...
              <div className="field" style={{ flex: '2 1 240px' }}>
                ...
                <input name="itemName" value={menuForm.itemName} onChange={handleMenuFormChange} required placeholder="e.g. Chicken Biryani" />
              </div>
            ...
            <div className="field">
              ...
              <input name="description" value={menuForm.description} onChange={handleMenuFormChange} placeholder="e.g. Served with raita and salad" />
            </div>
            <div className="field" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <input type="checkbox" id="isPublished" name="isPublished" checked={menuForm.isPublished} onChange={handleMenuFormChange} style={{ width: 'auto' }} />
              <label htmlFor="isPublished" style={{ margin: 0 }}>Published (visible to employees)</label>
            </div>
            {menuError && <p className="msg-error">{menuError}</p>}
            {menuMessage && <p className="msg-success">{menuMessage}</p>}
            <div style={{ display: 'flex', gap: '0.75rem' }}>
              <button type="submit" className="btn" disabled={savingMenu}>{savingMenu ? 'Saving…' : 'Save menu'}</button>
              <button type="button" className="btn btn-quiet" onClick={() => setMenuForm(emptyMenuForm)}>Clear</button>
            </div>
          </form>
        </div>
        ...
        {weekMenus.length === 0 ? (
          ...
          <table className="data-table">
            <thead><tr><th>Date</th><th>Dish</th><th>Price</th><th>Published</th><th></th></tr></thead>
            <tbody>
              {weekMenus.map((m) => (
                <tr key={m.id}>
                  ...
                  <td><button className="btn btn-quiet btn-sm" onClick={() => editMenu(m)}>Edit</button></td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <form className="panel-row" onSubmit={handleVendorSubmit}>
          ...
          <div className="field" style={{ marginBottom: 0 }}>
            <input name="contactInfo" value={vendorForm.contactInfo} onChange={handleVendorFormChange} placeholder="Contact (phone/email, optional)" />
          </div>
          <button type="submit" className="btn" disabled={savingVendor}>{savingVendor ? 'Adding…' : 'Add vendor'}</button>
        </form>
        ...
        {vendors.length > 0 && (
          <table className="data-table">
            ...
            <tbody>
              {vendors.map((v) => (
                <tr key={v.id}><td>{v.name}</td><td>{v.contactInfo || '—'}</td></tr>
              ))}
            </tbody>
          </table>
        )}
            ...
            <tbody>
              {users.map((u) => (
                <tr key={u.id}>
                  ...
                  <td>
                    {u.isActive && (
                      <button className="btn btn-danger btn-sm" onClick={() => handleDeleteUser(u)}>Delete</button>
                    )}
                  </td>
                </tr>
              ))}
        ...
        <div className="filter-row">
          <div className="chip-group">
            <button type="button" className={`chip ${filterMode === 'today' ? 'active' : ''}`} onClick={() => setFilterMode('today')}>Today</button>
            <button type="button" className={`chip ${filterMode === 'yesterday' ? 'active' : ''}`} onClick={() => setFilterMode('yesterday')}>Yesterday</button>
            <button type="button" className={`chip ${filterMode === 'custom' ? 'active' : ''}`} onClick={() => setFilterMode('custom')}>Custom range</button>
          </div>
          {filterMode === 'custom' && (
            <>
              <div className="field">
        ...
        <div className="tab-row">
          <button className={`tab-btn ${reportView === 'user' ? 'active' : ''}`} onClick={() => setReportView('user')}>User-wise</button>
          <button className={`tab-btn ${reportView === 'vendor' ? 'active' : ''}`} onClick={() => setReportView('vendor')}>Vendor-wise</button>
        </div>
        ...
        {reportView === 'user' ? (
          <>
            <div className="panel-row">
              ...
              {assignMessage && <span className="msg-success" style={{ margin: 0 }}>{assignMessage}</span>}
            </div>
              ...
              <table className="data-table">
                <thead>
                  <tr>
                    <th>
                      <input
                        type="checkbox"
                        checked={selectedOrderIds.length === ordersData.orders.length && ordersData.orders.length > 0}
                ...
                <tbody>
                  {ordersData.orders.map((o) => (
                    <tr key={o.id}>
                      <td><input type="checkbox" checked={selectedOrderIds.includes(o.id)} onChange={() => toggleSelectOne(o.id)} /></td>
                      ...
                      <td>
                        <button className="btn btn-quiet btn-sm" disabled={!rowAssignVendor || assigning} onClick={() => assignToOrders([o.id])}>
                          Assign
                        </button>
                      </td>
                    </tr>
          ...
          <table className="data-table">
            ...
            <tbody>
              {vendorRows.map((r) => (
                <tr key={r.vendor}><td>{r.vendor}</td><td>{r.quantity}</td><td>Rs. {r.amount.toFixed(2)}</td></tr>
              ))}
            </tbody>
            <tfoot>
              <tr>
        ...
        <div className="panel-row">
          ...
          <button onClick={handleExport} className="btn btn-quiet">Download CSV for this date</button>
        </div>
      </div>
    </div>
  );
